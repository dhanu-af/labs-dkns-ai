...
export function SpecTable({ specifications }: { specifications: Prisma.JsonValue }) {
  ...
  return (
    <dl className="grid grid-cols-1 gap-x-6 gap-y-3 rounded-lg border border-black/10 p-4 sm:grid-cols-2 dark:border-white/10">
      {entries.map(([key, value]) => (
        <div key={key} className="flex flex-col">
          <dt className="text-xs font-medium uppercase tracking-wide text-black/40 dark:text-white/40">{key}</dt>
