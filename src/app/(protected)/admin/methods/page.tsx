...
import { deleteMethod } from "@/lib/actions/admin-actions";
import { DeleteButton } from "@/components/admin/DeleteButton";

export const metadata: Metadata = { title: "Admin — Methods" };
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Methods</h1>
        <Link href="/admin/methods/new" className="app-hero-glow rounded-full px-4 py-2 text-sm font-semibold text-white">
          + New method
        </Link>
      </div>
      ...
      <div className="mt-6 divide-y divide-slate-900/[0.07] rounded-2xl border border-slate-900/[0.07] bg-white dark:divide-white/10 dark:border-white/10 dark:bg-white/[0.03]">
        {items.length === 0 ? (
          <p className="p-5 text-sm text-slate-500 dark:text-white/50">No methods yet.</p>
        ) : (
          items.map((item) => (
            <div key={item.id} className="flex items-center justify-between gap-4 p-4">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <DeleteButton action={deleteMethod.bind(null, item.id)} />
              </div>
            </div>
          ))
        )}
